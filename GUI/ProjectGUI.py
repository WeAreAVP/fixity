...
class ProjectGUI(GUILibraries.QMainWindow):
    ...
    def Save(self, project = None):
        ...
        if self.Fixity.Configuration.getOsType() == 'Windows':
            if response[0] == 1:
                self.notification.showInformation(self, "Warning", GUILibraries.messages['settings_saved'] + self.projects.currentItem().text() + GUILibraries.messages['settings_saved_but'])
            else:
