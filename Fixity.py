...
class ProjectWin(QMainWindow):
        ...
        '''
        def buildTable(self, roorDirectory, algorithum):
            ...
            for root, subFolders, files in walk(roorDirectory):
                    subFolders
                    for filePath in files:
                            fls.append(path.join(root, filePath))
            ...
            for f in xrange(len(fls)):
                    ...
                    pathOfFileInThisDir = path.abspath(fls[f])
                    hashOfFile = ''
                    if(OS_Info == 'Windows'):
                        hashOfFile = FixityCore.fixity(pathOfFileInThisDir, algorithum)
                    else:
                        hashOfFile = FixityCore.fixity(pathOfFileInThisDir, algorithum)

                    if(OS_Info == 'Windows'):
                        inode = FixityCore.FixityCoreWin.ntfsIDForWindows(pathOfFileInThisDir)
                    else:
                        inode = FixityCore.FixityCoreMac.ntfsIDForMac(pathOfFileInThisDir)

                    listOfDirectories.append((hashOfFile, pathOfFileInThisDir, inode))
